...
class TargetDetection():
	...
	def verbose_detect_target(self, img):
		"""
		Log Keys:
		Initial Image
		Resized Image
		...
		Result (0 or 1 as determined by the SVM)
		Output (what would get returned normally by detect_target)
		
		Status (either contains "Worked" or the error message)
		Flags (tells you how far through the try block it got)
		"""
		flags = [0,0,0]
		log = {}
		log['Initial Image'] = img
	
		res_scale = 0.5
		img = cv2.resize(img, (0,0), fx=res_scale, fy=res_scale)
		log['Resized Image'] = img
		
		objmask = image_to_mask(img)
		log['Object Mask'] = objmask
		flags[0] = 1
		...
		try:
			...
			svm_result = self.svm(features)
			log['Result'] = svm_result
			flags[2] = 2
			
			log['Status'] = 'Worked'
			
			log['Output'] = (features['Centroid',svm_result))
			log['Flags'] = flags
